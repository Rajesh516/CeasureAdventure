...
public class GUIManager : MonoBehaviour 
{
    ...
    public TextMesh[] InGameTexts;                 //The main GUI coin and distance indicator

             //The finish menu coin and distance indicators
	...
	void Update()
	{
		...
		if (showMainGUI && !showPause)
		{
            ...
            DisplayStat(InGameTexts[0], LevelManager.Instance.Coins(), 4);
			DisplayStat(InGameTexts[1], (int)LevelGenerator.Instance.distance, 5);
		}
	}
	...
	void AuidoTriggerer(GameObject audioButton)
	{
    ...
    IEnumerator Resume()
    {
		...
		UIManager.Instance.ShowResumeScreen ();
    }
    ...
    IEnumerator QuitToMain()
    {
		...
		StartCoroutine(FadeScreen(0.1f, 0.0f));
	...
	public void ButtonUp(Transform button)
	{
		...
		case "ShareFacebook":
			...
			break;
		case "ShareTwitter":
			TwitterGuiManager.Instance.Logintwitter ();
			break;
		case "WorldTopScore":
			//AuidoTriggerer(button.gameObject);
			break;
